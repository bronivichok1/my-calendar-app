    ...
    <div className="App">
      {!isAnyCalendarInDayView && (
        <div className="header">
          ...
          <h1 className="header-title">Кабинет №{number}</h1>
            ...
            <Calendar
              dayPropGetter={(date) => {
                ...
                const calendarViewDate = moment(calendarDisplayStates[index].date);
                 if (calendarDisplayStates[index].view === 'day' && momentDate.isSame(calendarViewDate, 'day')) {
                    return { className: 'active-day' };
                 }
                 if (!isDayView && selectedMonthIndex === index && highlightedDate && momentDate.isSame(highlightedDate, 'day')) {
                    return { className: 'highlighted-date' };
                 }
                 if (!isDayView && momentDate.isSame(calendarDisplayStates[index].date, 'month')) {
                    if (!momentDate.isBefore(now, 'day')) {
                      return { className: 'selectable-day' };
                    }
                 }
                 return { className: 'rbc-day-bg' };
               }}
              toolbar={false}
              localizer={localizer}
                ...
                style={{
                  ...
                  <div className="frame" style={{ marginTop: '20px' }}>
                    <button
                      ...
                      type="submit"
                      disabled={!isStartDateValid || !isEndDateValid || !eventTitle || !eventName}
                    >
                      Добавить
                    </button>
                  </div>
